...
export async function queryLatestCycleRecordFromArchiver(ip, port, cycleCount) {
    ...
    if (res.data.cycleInfo.length > 0)
        if (cycleCount === 1)
            return res.data.cycleInfo[0];
        else
            return res.data.cycleInfo;
    else
        return null;
}
...
export async function waitForNetworkToBeActive(numberOfExpectedNodes) {
    ...
    while (!ready) {
        try {
            ...
            if (activeNodes) {
                if (Object.keys(activeNodes).length >= numberOfExpectedNodes)
                    ready = true;
            }
        }
        catch (e) {
        ...
        if (!ready)
            await _sleep(10000 * Math.max(attempt / 2, 1));
        if (attempt === numberOfExpectedNodes * 3)
            break; // 3 attempts is in one cycle, total cycle is number of Nodes
        attempt++;
    }
    return ready;
}
export async function waitForArchiverToJoin(ip: string, port: number) {
    ...
    while (!ready) {
        try {
            ...
            if (newArchiver) {
                if (newArchiver.ip === ip && newArchiver.port === port)
                    ready = true;
            }
        }
        catch (e) {
