 ...
 */
async function exportData(dbFiles, batchSize: number, targetDBPath: string, targetJSONPath: string, exportToJson: boolean) {
...
async function createTargetDB(targetDBPath: string) {
    ...
    targetDB.run('CREATE TABLE if not exists `accountsEntry` (`accountId` VARCHAR(255) NOT NULL, `timestamp` BIGINT NOT NULL, `data` JSON NOT NULL, PRIMARY KEY (`accountId`))')
    await sleep(1000)
