...
config = merge(config, {
  server: {
    p2p: {
      ...
      baselineNodes: process.env.baselineNodes ? parseInt(process.env.baselineNodes) : 128, // config used for baseline for entering recovery, restore, and safety. Should be equivalient to minNodes on network startup
      minNodes: process.env.minNodes ? parseInt(process.env.minNodes) : 128,
