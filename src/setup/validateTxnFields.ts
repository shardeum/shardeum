...
export const validateTxnFields =
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    (
      ...
      try {
        //force this to run once per tx, so that we dont trust what was sent over the network
        //a bit ineffienct if we are then node that got the inject
        let transaction = null // originalAppData?.localTxCache
        if (transaction == null) {
          transaction = getTransactionObj(tx)
          originalAppData.localTxCache = transaction
        } else {
        ...
        if (appData && appData.internalTx && appData.internalTXType === InternalTXType.Unstake) {
          ...
          if (
            ...
            nestedCountersInstance.countEvent(
              'shardeum-unstaking',
              'invalid nominee address in stake coins tx'
            )
            success = false
            reason = `Invalid nominee address in stake coins tx`
          }
