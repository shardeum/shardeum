        ...
        if (shardus.p2p.isFirstSeed) {
            ...
            if (ShardeumFlags.SetupGenesisAccount) {
                ...
                for (const address in mergedGenesisAccounts) {
                    ...
                    const { wrappedEVMAccount, accountId, cycle } = await manuallyCreateAccount(ethAccountID, amount, evmCommon, shardus.getLatestCycles());
                ...
                for (const secureAccountConfig of genesisSecureAccounts) {
                    ...
                    accountCopies.push(accountCopy);
                    /* prettier-ignore */ if (logFlags.important_as_error)
                        shardus.log(`node ${nodeId} SETUP GENESIS SECUREACCOUNT: ${secureAccount.id}`);
                }
 ...
 * @param balance
 */
async function manuallyCreateAccount(ethAccountID: string, balance = defaultBalance, evmCommon: any, latestCycles: any // eslint-disable-line @typescript-eslint/no-explicit-any
): Promise<{
    accountId: string;
    wrappedEVMAccount: {
        timestamp: number;
