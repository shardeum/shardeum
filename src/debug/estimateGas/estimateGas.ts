...
export async function estimateGas(txData, preRunTxState: ShardeumState, wrappedStates: Map<string, WrappedEVMAccount>, EVM): Promise<void> {
    ...
    if (transaction.to == null) {
        ...
        if (senderWrappedEVMAccount) {
            ...
            preRunTxState._transactionState.insertFirstAccountReads(Address.fromString(predictedContractAddressString), createdAccount.account);
        }
    }
    ...
    preRunTxState._transactionState.insertFirstAccountReads(senderAddress, callerAccount ? callerAccount.account : fakeAccount);
