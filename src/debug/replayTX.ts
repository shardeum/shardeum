...
const runTransaction = async (txJson, wrappedStates: Map<string, WrappedEVMAccount>, execOptions: {
    ...
    if (transaction.to == null) {
        ...
        if (senderWrappedEVMAccount) {
            ...
            shardeumState._transactionState.insertFirstAccountReads(Address.fromString(predictedContractAddressString), createdAccount.account);
    ...
    const stepListener = async (event: InterpreterStep, next: (error?: Error | null, cb?: () => void) => void): Promise<void> => {
        try {
