...
import { Account, Address, KECCAK256_NULL, bytesToHex, bytesToUnprefixedHex, equalsBytes, hexToBytes, short, } from '@ethereumjs/util';
...
import type { AfterTxEvent, BaseTxReceipt, EIP4844BlobTxReceipt, PostByzantiumTxReceipt, PreByzantiumTxReceipt, RunTxOpts, RunTxResult, TxReceipt, } from './types.js';
import type { VM } from './vm.js';
import type { AccessList, AccessListItem } from '@ethereumjs/common';
import type { AccessListEIP2930Transaction, FeeMarketEIP1559Transaction, LegacyTransaction, TypedTransaction, } from '@ethereumjs/tx';
...
export async function runTx(this: VM, opts: RunTxOpts, evm: EthereumVirtualMachine, txid: string): Promise<RunTxResult> {
    ...
    if (BigInt(chainId) !== BigInt(ShardeumFlags.ChainID)) {
        await evm.journal.revert();
        const msg = _errorMsg(`Invalid chainId: expected ${ShardeumFlags.ChainID}, got ${chainId}`, this, opts.block, opts.tx);
        throw new Error(msg);
    }
    try {
 ...
 */
export async function generateTxReceipt(this: VM, tx: TypedTransaction, txResult: RunTxResult, cumulativeGasUsed: bigint, blobGasUsed?: bigint, blobGasPrice?: bigint): Promise<TxReceipt> {
