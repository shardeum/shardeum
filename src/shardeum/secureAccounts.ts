...
export function initializeSecureAccount(secureAccountConfig: SecureAccountConfig, latestCycles: {
    start: number;
}[]): SecureAccount {
    let cycleStart = 0;
...
export const secureAccountDataMap: Map<string, SecureAccountData> = new Map(genesisSecureAccounts.map(account => [account.Name, account]));
...
export function validateTransferFromSecureAccount(tx: InternalTx, shardus: Shardus): {
    success: boolean;
    reason: string;
} {
