...
class Storage {
    ...
    async getRIAccountsCache(accountId: string): Promise<AccountsEntry> {
        this._checkInit();
        try {
            const result = await this._read(this.storageModels.riAccountsCache, { accountId }, {
    ...
    async getAccountsEntry(accountId): Promise<AccountsEntry> {
        this._checkInit();
        try {
            const result = await this._read(this.storageModels.accountsEntry, { accountId }, {
                attributes: { exclude: ['createdAt', 'updatedAt', 'id'] },
                raw: true,
            });
            if (result.length > 0) {
    ...
    async queryAccountsEntryByRanges3(accountStart: string, accountEnd: string, tsStart: number, tsEnd: number, limit: number, accountOffset: string): Promise<AccountsEntry[]> {
    ...
    async queryAccountsEntryByRanges2(accountStart: string, accountEnd: string, tsStart: number, tsEnd: number, limit: number, offset: number): Promise<AccountsEntry[]> {
        this._checkInit();
        try {
            const result = await this._read(this.storageModels.accountsEntry, {
    ...
    async queryAccountsEntryByRanges(accountStart: string, accountEnd: string, limit: number
    //offset:number
