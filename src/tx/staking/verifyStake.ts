...
import { AccountType, NetworkAccount, NodeAccount2, StakeCoinsTX, UnstakeCoinsTX, WrappedEVMAccount, WrappedStates, } from '../../shardeum/shardeumTypes';
...
export function verifyStakeTx(appData: any, senderAddress: Address, wrappedStates: WrappedStates): {
    success: boolean;
    reason: string;
} {
    let success = true;
    ...
    if (stakeCoinsTx.nominator == null || stakeCoinsTx.nominator.toLowerCase() !== senderAddress.toString()) {
        ...
        success = false;
        reason = `This node was staked within the last ${networkAccount.current.restakeCooldown / 60000} minutes. You can't stake more to this node yet!`;
    }
...
export function verifyUnstakeTx(appData: any, senderAddress: Address, wrappedStates: WrappedStates, shardus: Shardus): {
    success: boolean;
    reason: string;
} {
    let success = true;
    ...
    if (!nominatorAccount) {
        ...
        else if (nomineeAccount.rewardEndTime === 0 &&
...
export function isStakeUnlocked(nominatorAccount: WrappedEVMAccount, nomineeAccount: NodeAccount2, shardus: Shardus, networkAccount: NetworkAccount): {
    unlocked: boolean;
    reason: string;
