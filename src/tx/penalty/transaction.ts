...
export async function injectPenaltyTX(
  ...
  const wrapeedNodeAccount: ShardusTypes.WrappedDataFromQueue = await shardus.getLocalOrRemoteAccount(tx.reportedNodePublickKey)
...
export function validatePenaltyTX(tx: PenaltyTX, shardus: Shardus): { isValid: boolean; reason: string } {
  ...
  if (tx.violationType === ViolationType.LeftNetworkEarly) {
    ...
    const droppedCycleRecord = latestCycles.find(record => record.counter === violationData.nodeDroppedCycle)
...
export async function applyPenaltyTX(
  ...
  if (!isValidRequest) {
    /* prettier-ignore */
    console.log(`Invalid penaltyTX, reportedNode ${tx.reportedNodePublickKey}, reason: ${isValidRequest.reason}`)
  ...
  if (ShardeumFlags.supportInternalTxReceipt) {
    createInternalTxReceipt(shardus, applyResponse, tx, tx.reportedNodePublickKey, tx.reportedNodePublickKey, txTimestamp, txId)
  }

  /* prettier-ignore */
  nestedCountersInstance.countEvent('shardeum-penalty', `Applied PenaltyTX`)
  console.log('Applied PenaltyTX', tx.reportedNodePublickKey)
}
