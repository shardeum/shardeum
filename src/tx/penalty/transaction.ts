...
import { AccountType, InternalTXType, isNodeAccount2, LeftNetworkEarlyViolationData, NodeAccount2, NodeRefutedViolationData, PenaltyTX, SyncingTimeoutViolationData, ViolationType, WrappedEVMAccount, WrappedStates, } from '../../shardeum/shardeumTypes';
...
export async function injectPenaltyTX(shardus: Shardus, eventData: ShardusTypes.ShardusEvent, violationData: LeftNetworkEarlyViolationData | NodeRefutedViolationData | SyncingTimeoutViolationData): Promise<{
    success: boolean;
    reason: string;
    status: number;
}> {
 ...
 */
function isProcessedPenaltyTx(tx: PenaltyTX, nodeAccount: NodeAccount2): {
    isProcessed: boolean;
    eventTime: number;
} {
...
export function validatePenaltyTX(txId: string, tx: PenaltyTX, isApply = false): {
    isValid: boolean;
    reason: string;
} {
