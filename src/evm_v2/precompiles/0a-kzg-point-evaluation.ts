import { bigIntToBytes, bytesToHex, computeVersionedHash, concatBytes, kzg, setLengthLeft, short, } from '@ethereumjs/util';
...
export async function precompile0a(opts: PrecompileInput): Promise<ExecResult> {
    ...
    if (opts._debug !== undefined) {
        opts._debug(`Run KZG_POINT_EVALUATION (0x14) precompile data=${short(opts.data)} length=${opts.data.length} gasLimit=${opts.gasLimit} gasUsed=${gasUsed}`);
    }
    ...
    if (opts._debug !== undefined) {
        opts._debug(`KZG_POINT_EVALUATION (0x14): proof verification with commitment=${bytesToHex(commitment)} z=${bytesToHex(z)} y=${bytesToHex(y)} kzgProof=${bytesToHex(kzgProof)}`);
    }
    try {
    ...
    if (opts._debug !== undefined) {
        opts._debug(`KZG_POINT_EVALUATION (0x14) return fieldElements=${bytesToHex(fieldElementsBuffer)} modulus=${bytesToHex(modulusBuffer)}`);
    }
    return {
