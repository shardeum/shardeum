...
export function encodeDaoTxToEVMTx(daoTx: { type: string, [key: string]: unknown}, shardus: Shardus): string {
  ...
  const shardusSecretKey = shardus.crypto.keypair.secretKey
  const ethSecretKey = '0x' + shardusSecretKey.substring(0, 64)
  const signedTx = tx.sign(hexToBytes(ethSecretKey))
