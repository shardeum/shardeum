...
import { IDevIssue } from './dev_issue'
...
export class Tally implements ITally, DaoTx<NodeAccount | DevIssueAccount> {
  readonly type = 'tally'
  nodeId: string
  from: string
  issue: string
  ...
  validateFields(response: ShardusTypes.IncomingTransactionResult): ShardusTypes.IncomingTransactionResult {
    if (typeof this.nodeId !== 'string') {
      ...
      response.reason = 'tx "issue" field must be a string.'
      throw new Error(response.reason)
    } else if (!Array.isArray(this.proposals)) {
      response.success = false
      response.reason = 'tx "proposals" field must be an array.'
      throw new Error(response.reason)
    }
  ...
  validate(
    ...
    const network: DaoGlobalAccount = wrappedStates[daoConfig.daoAccountAddress].data
    const issue: IssueAccount = wrappedStates[ensureShardusAddress(this.issue)]?.data
    const proposals: ProposalAccount[] = this.proposals.map((id: string) => wrappedStates[id].data)
    ...
    if (network.windows.graceWindow.excludes(this.timestamp)) {
      response.reason = 'Network is not within the time window to tally votes for proposals'
      return response
    }
    response.success = true
  ...
  apply(
    ...
    const sortedProposals: ProposalAccount[] = this.proposals
      .map((id: string) => wrappedStates[id].data)
      .sort((a: ProposalAccount, b: ProposalAccount) => b.power - a.power)
    let winner = defaultProposal
  ...
  keys(result: TransactionKeys): TransactionKeys {
    ...
    result.targetKeys = [...this.proposals, this.issue, daoConfig.daoAccountAddress]
    result.allKeys = [...result.sourceKeys, ...result.targetKeys]
    return result
  }
