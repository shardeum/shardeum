...
export function keys(tx: IDevTally, result: TransactionKeys): TransactionKeys {
...
export class DevTally implements IDevTally, DaoTx<NodeAccount> {
  readonly type = 'dev_tally'
  nodeId: string
  from: string
  devIssue: string
  devProposals: string[]
  timestamp: number
  constructor(data: IDevTally) {
    this.nodeId = data.nodeId
    this.from = data.from
    this.devIssue = data.devIssue
    this.devProposals = data.devProposals
  ...
  validateFields(response: ShardusTypes.IncomingTransactionResult): ShardusTypes.IncomingTransactionResult {
    if (typeof this.nodeId !== 'string') {
      ...
      response.reason = 'tx "devIssue" field must be a string.'
      throw new Error(response.reason)
    } else if (!Array.isArray(this.devProposals)) {
      response.success = false
      response.reason = 'tx "devProposals" field must be an array.'
      throw new Error(response.reason)
    }
  ...
  validate(
    ...
    const devProposals: DevProposalAccount[] = this.devProposals.map((id: string) => wrappedStates[id].data)
    ...
    if (network.devWindows.graceWindow.excludes(this.timestamp)) {
      response.reason = 'Network is not within the time window to tally votes for developer proposals'
      return response
    }
    response.success = true
      ...
      if (devProposal.approve > devProposal.reject + devProposal.reject * 0.15) {
        ...
        for (const payment of devProposal.payments) {
          payments.push({
            ...
            delay: payment.delay,
            amount: payment.amount * (devProposal.totalAmount ?? 0),
            address: ensureShardusAddress(devProposal.payAddress),
            id: crypto.hashObj(payment),
          })
        }
        nextDeveloperFund = [...nextDeveloperFund, ...payments]
  ...
  keys(result: TransactionKeys): TransactionKeys {
    ...
    result.targetKeys = [...this.devProposals, this.devIssue, daoConfig.daoAccountAddress]
    result.allKeys = [...result.sourceKeys, ...result.targetKeys]
    return result
  }
  ...
  createRelevantAccount(
    dapp: Shardus,
    account: NodeAccount | DevProposalAccount,
    accountId: string,
    accountCreated = false
  ): WrappedResponse {
    if (!account) {
