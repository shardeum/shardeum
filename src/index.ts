...
dapp.setup({
    ...
    validateTxnFields(txTs) {
        ...
        if (tx.timestamp) txnTimestamp = tx.timestamp
        else if (tsReceipt && tsReceipt.timestamp) txnTimestamp = tsReceipt.timestamp
        else {
            success = false
            reason = 'No injected timestamp or timestamp receipt found!'
        }
    ...
    getTimestampFromInjectedTx(injectedTx) {
        return injectedTx.timestamp
    },
    async apply(txTs, wrappedStates) {
        let { tx, tsReceipt } = txTs
        // Validate the tx
    ...
    async getRelevantData(accountId, txTs) {
        let { tx, tsReceipt } = txTs
