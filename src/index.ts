...
const configShardusEndpoints = (): void => {
  ...
  shardus.registerExternalGet('stake', async (req, res) => {
    try {
      ...
      return res.json(Utils.safeJsonParse(Utils.safeStringify({ stakeRequired, stakeRequiredUsd })))
    } catch (e) {
...
const shardusSetup = (): void => {
  ...
  shardus.setup({
    ...
    async apply(timestampedTx: ShardusTypes.OpaqueTransaction, wrappedStates, originalAppData) {
      ...
      const wrappedNetworkAccount: ShardusTypes.WrappedData =
        await shardus.getLocalOrRemoteAccount(networkAccount)
    ...
    async getRelevantData(accountId, timestampedTx, appData) {
      ...
      if (isInternalTx(tx)) {
        ...
        let wrappedEVMAccount: NetworkAccount | WrappedEVMAccount =
          await AccountsStorage.getAccount(accountId)
        shardus.setDebugSetLastAppAwait(
    ...
    async updateAccountFull(wrappedData, localCache, applyResponse: ShardusTypes.ApplyResponse) {
      ...
      console.log('RED-153 setAccount', accountId, 'hash:', updatedEVMAccount.hash, 'trace:', new Error())
    ...
    async signAppData(
      ...
      try {
        ...
        if (type === 'sign-stake-cert') {
          ...
          if (ShardeumFlags.VerboseLogs)
            console.log(`signAppData passed ${type} ${Utils.safeStringify(stakeCert)}`)
