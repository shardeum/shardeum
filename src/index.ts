      ...
      if (isInternalTx(tx)) {
        ...
        if (internalTx.internalTXType === InternalTXType.SetCertTime) {
          if (!wrappedEVMAccount) {
            ...
            if (accountId === internalTx.nominee) {
              throw Error(
                `Node Account <nominee> is not found ${accountId}, tx: ${JSON.stringify(internalTx)}`
              )
    ...
    async signAppData(
      ...
      try {
        ...
        if (type === 'sign-stake-cert') {
          ...
          if (ShardeumFlags.VerboseLogs)
            console.log(`signAppData passed ${type} ${JSON.stringify(stakeCert)}`)
    ...
    validateJoinRequest(data, mode: P2P.ModesTypes.Record['mode'] | null)  {
      try {
        ...
        if (
          ...
          /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log(`validateJoinRequest ${JSON.stringify(stake_cert)}`)
