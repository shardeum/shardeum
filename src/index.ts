...
import { safeStringify, safeJsonParse } from '@shardeum-foundation/lib-types/build/src/utils/functions/stringify'
...
function convertToReadableBlock(block: Block): ShardeumBlockOverride {
  const defaultBlock = {
    ...
    logsBloom:
      '',
...
export function getStakeTxBlobFromEVMTx(
  ...
  return safeJsonParse(stakeTxString)
}
  ...
  try {
    ...
    return {
      ...
      operatorAccountInfo: account.operatorAccountInfo
        ? safeJsonParse(safeStringify(account.operatorAccountInfo))
        : null,
    }
  } catch (e) {
...
function logAccessList(message: string, appData): void {
  if (appData != null && appData.accessList != null) {
    if (ShardeumFlags.VerboseLogs) console.log(`access list for ${message} ${safeStringify(appData.accessList)}`)
  }
}
...
const configShardusEndpoints = (): void => {
  ...
  shardus.registerExternalGet('debug-points', debugMiddleware, async (req, res) => {
    try {
      ...
      res.json(
        `spent points: ${points} total:${debugLastTotalServicePoints}  ${safeStringify(
  ...
  shardus.registerExternalGet('canUnstake/:nominee/:nominator', externalApiMiddleware, async (req, res) => {
    ...
    try {
      ...
      if (nominee == null || nominator == null || nominator.account == null || nominee.account == null) {
        ...
        if (accountIsRemote) {
          ...
          if (consensusNode != null) {
            ...
            if (postResp != null && postResp.body != null && postResp.body != '') {
              ...
              /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log(`Node is in remote shard: gotResp:${safeStringify(postResp.body)}`)
  ...
  shardus.registerExternalGet('debug-appdata/:hash', debugMiddleware, async (req, res) => {
    try {
      ...
      if (appData == null) {
        res.json(safeStringify({ result: `no appData` }))
        return
      }

      //return res.json(`${Utils.safeStringify(appData)}`)

      res.write(safeStringify(appData, null))
  ...
  shardus.registerExternalGet('accounts', debugMiddlewareMedium, async (req, res) => {
    try {
      ...
      const sorted = safeJsonParse(safeStringify(accounts))
  ...
  shardus.registerExternalPut('query-certificate', externalApiMiddleware, async (req: Request, res: Response) => {
    try {
      ...
      res.json(safeJsonParse(safeStringify(queryCertRes)))
    } catch (error) {
      const message = error && error.message ? error.message : 'Internal server error'
      res.status(500).json({ error: message })
    }
...
const configShardusNetworkTransactions = (): void => {
  shardus.serviceQueue.registerBeforeAddVerifier(
    ...
    async (txEntry: P2P.ServiceQueueTypes.AddNetworkTx<SignedNodeRewardTxData>) => {
      ...
      /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('Validating nodeReward fields', safeStringify(tx))
      try {
        if (!crypto.verifyObj(tx)) {
          /* prettier-ignore */
          if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerifier - nodeReward: fail Invalid signature', safeStringify(tx))
          return false
        }
      } catch (e) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('Invalid signature for internal tx', safeStringify(tx))
        return false
      }
      ...
      if (txEntry.priority !== 0) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerifier - nodeReward: fail Invalid priority', safeStringify(tx))
        return false
      }
      if (txEntry.subQueueKey == null || txEntry.subQueueKey != tx.publicKey) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerifier - nodeReward: fail Invalid subQueueKey', safeStringify(tx))
        return false
      }
      if (!tx.publicKey || tx.publicKey === '' || tx.publicKey.length !== 64) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerify nodeReward fail invalid publicKey field', safeStringify(tx))
      ...
      if (nodePubKey == null || tx.publicKey !== nodePubKey) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerify nodeReward fail invalid nodeId field', safeStringify(tx))
      ...
      if (tx.endTime === undefined) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerify nodeReward fail endTime field missing', safeStringify(tx))
      ...
      if (!nodeDeactivatedCycle) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerify nodeReward fail !nodeDeactivatedCycle', safeStringify(tx))
      ...
      if (tx.endTime < removeCycleRange[0] || tx.endTime > removeCycleRange[1]) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerify nodeReward fail endTime is not in range', safeStringify(tx))
      ...
      if (!nodeRemovedCycle) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerify nodeReward fail !nodeRemovedCycle', safeStringify(tx))
      ...
      /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerify nodeReward success', safeStringify(tx))
      return true
    }
  )
  shardus.serviceQueue.registerApplyVerifier(
    ...
    async (txEntry: P2P.ServiceQueueTypes.AddNetworkTx<SignedNodeRewardTxData>) => {
      const tx = txEntry.txData
      /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('Validating nodeReward applied', safeStringify(tx))
      const shardusAddress = tx.publicKey?.toLowerCase()
      const account = await shardus.getLocalOrRemoteAccount(shardusAddress)
      if (!account) {
        console.log(
  ...
  shardus.serviceQueue.registerBeforeAddVerifier(
    ...
    async (txEntry: P2P.ServiceQueueTypes.AddNetworkTx<SignedNodeInitTxData>) => {
      ...
      if (!isValid) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('validate nodeInitReward fail Invalid signature', safeStringify(tx))
      ...
      if (txEntry.subQueueKey == null || txEntry.subQueueKey != tx.publicKey) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerifier - nodeInitReward: fail Invalid subQueueKey', safeStringify(tx))
        return false
      }

      const node = shardus.getNode(tx.nodeId)
      if (node == null || tx.publicKey !== node.publicKey) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('registerBeforeAddVerify nodeInitReward fail invalid nodeId field', safeStringify(tx))
      ...
      if (!nodeActivedCycle) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('validate nodeInitReward fail !nodeActivedCycle', safeStringify(tx))
      ...
      if (nodeActivedCycle.start !== tx.startTime) {
        /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('validate nodeInitReward fail nodeActivedCycle.start !== tx.nodeActivatedTime', safeStringify(tx))
      ...
      /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('validate nodeInitReward success', safeStringify(tx))
      return true
    }
  )
  shardus.serviceQueue.registerApplyVerifier(
    ...
    async (txEntry: P2P.ServiceQueueTypes.AddNetworkTx<SignedNodeInitTxData>) => {
      const tx = txEntry.txData
      /* prettier-ignore */ if (ShardeumFlags.VerboseLogs) console.log('Validating nodeInitReward applied', safeStringify(tx))
      const shardusAddress = tx.publicKey?.toLowerCase()
      const account = await shardus.getLocalOrRemoteAccount(shardusAddress)
      if (!account) {
        console.log(
      ...
      if (txListEntry && txListEntry.tx.type === 'nodeReward') {
        /** prettier-ignore */ if (ShardeumFlags.VerboseLogs)
          console.log(
            ...
            safeStringify(txListEntry)
          )
        return
      }
      /** prettier-ignore */ if (ShardeumFlags.VerboseLogs)
        console.log(`Creating a shutdown reward tx`, safeStringify(txListEntry), safeStringify(node))
      return {
        type: 'nodeReward',
        txData: {
...
async function applyInternalTx(
  ...
  if (internalTx.internalTXType === InternalTXType.InitNetwork) {
    ...
    /* prettier-ignore */ if (logFlags.important_as_error) console.log(`init_network NETWORK_ACCOUNT: ${safeStringify(network)}`)
  ...
  if (internalTx.internalTXType === InternalTXType.ChangeConfig) {
    ...
    const value = {
      ...
      change: { cycle: changeOnCycle, change: safeJsonParse(internalTx.config) },
    }
  ...
  if (internalTx.internalTXType === InternalTXType.ApplyChangeConfig) {
    ...
    /* prettier-ignore */ if (logFlags.important_as_error) console.log(`Applied CHANGE_CONFIG GLOBAL transaction: ${safeStringify(network)}`)
    /* prettier-ignore */ if (logFlags.important_as_error) shardus.log('Applied CHANGE_CONFIG GLOBAL transaction', safeStringify(network))
    if (ShardeumFlags.supportInternalTxReceipt) {
      createInternalTxReceipt(shardus, applyResponse, internalTx, internalTx.from, networkAccount, txTimestamp, txId)
    }
  }
  if (internalTx.internalTXType === InternalTXType.ChangeNetworkParam) {
    ...
    const value = {
      ...
      from: internalTx.from,
      network: networkAccount,
      change: { cycle: changeOnCycle, change: {}, appData: safeJsonParse(internalTx.config) },
    }
  ...
  if (internalTx.internalTXType === InternalTXType.ApplyNetworkParam) {
    ...
    /* prettier-ignore */ if (logFlags.important_as_error) console.log(`Applied CHANGE_NETWORK_PARAM GLOBAL transaction: ${safeStringify(network)}`)
    /* prettier-ignore */ if (logFlags.important_as_error) shardus.log('Applied CHANGE_NETWORK_PARAM GLOBAL transaction', safeStringify(network))
  }
...
async function _transactionReceiptPass(
  ...
  if (ourAppDefinedData.globalMsg) {
    ...
    if (ShardeumFlags.VerboseLogs) {
      ...
      console.log(`transactionReceiptPass setglobal: ${txHash} ${safeStringify(tx)}  `)
    }
  }
...
async function estimateGas(
  ...
  if (ShardeumFlags.VerboseLogs) {
    ...
    console.log('CALLER: ', safeStringify(callerAccount))
  }
...
async function generateAccessList(
  ...
  try {
      ...
      if (accountIsRemote && ShardeumFlags.debugLocalAALG === false) {
        ...
        if (consensusNode != null) {
          ...
          if (postResp != null && postResp.body != null && postResp.body != '' && postResp.body.accessList != null) {
            /* prettier-ignore */ if (logFlags.dapp_verbose || logFlags.aalg) console.log(`Node is in remote shard: gotResp:${safeStringify(postResp.body)}`)
    ...
    if (isEmptyCodeHash) {
      ...
      allInvolvedContracts:  ${safeStringify(allInvolvedContracts, null, 2)}

      readAccounts.contractBytes: ${safeStringify(readAccounts.contractBytes, null, 2)}`)
...
const shardusSetup = (): void => {
  ...
  shardus.setup({
    ...
    async apply(timestampedTx: ShardusTypes.OpaqueTransaction, wrappedStates, originalAppData) {
      ...
      if (result !== 'pass') {
        throw new Error(`invalid transaction, reason: ${reason}. tx: ${safeStringify(tx)}`)
      }
