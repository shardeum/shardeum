...
import { Account, Address, bytesToHex, bigIntToHex, isHexPrefixed, 
//fromAscii,
isValidAddress, toAscii, toBytes, hexToBytes, isHexString, } from '@ethereumjs/util';
import { AccessListEIP2930Transaction, LegacyTransaction, LegacyTxData, Transaction, TransactionFactory, TransactionType, } from '@ethereumjs/tx';
...
import { AccountType, AppJoinData, BlockMap, ClaimRewardTX, DebugTx, DebugTXType, 
//DevSecurityLevel,
//DevAccount,
EVMAccountInfo, InitRewardTimes, InternalTx, InternalTXType, isNodeAccount2, LeftNetworkEarlyViolationData, NetworkAccount, NodeAccount2, NodeInfoAppData, NodeInitTxData, NodeRefutedViolationData, NodeRewardTxData, OperatorAccountInfo, OperatorStats, OurAppDefinedData, PenaltyTX, ReadableReceipt, SecureAccountInfo, SetCertTime, ShardeumBlockOverride, SignedNodeInitTxData, SignedNodeRewardTxData, StakeCoinsTX, StakeInfo, SyncingTimeoutViolationData, UnstakeCoinsTX, WrappedAccount, WrappedEVMAccount, WrappedStates, } from './shardeum/shardeumTypes';
...
import { emptyCodeHash, isEqualOrNewerVersion, replacer, fixBigIntLiteralsToBigInt, sleep, zeroAddressStr, _base16BNParser, _readableSHM, scaleByStabilityFactor, isEqualOrOlderVersion, debug_map_replacer, operatorCLIVersion, operatorGUIVersion, readOperatorVersions, formatErrorMessage, calculateGasPrice, getRandom, findMajorityResult, generateTxId, isWithinRange, isValidVersion, getTxSenderAddress, isInSenderCache, removeTxFromSenderCache, isStakingEVMTx, convertBigIntsToHex, } from './utils';
...
import { CertSignaturesResult, InjectTxToConsensor, queryCertificate, queryCertificateHandler, RemoveNodeCert, StakeCert, ValidatorError, } from './handlers/queryCertificate';
import * as InitRewardTimesTx from './tx/initRewardTimes';
import * as PenaltyTx from './tx/penalty/transaction';
import { isDebugTx, isInternalTx, crypto, getInjectedOrGeneratedTimestamp, verifyMultiSigs, verify, } from './setup/helpers';
...
import { crack as crackTransferFromSecureAccount, apply as applyTransferFromSecureAccount, verify as verifyTransferFromSecureAccount, secureAccountDataMap } from './shardeum/secureAccounts';
import * as TicketManager from './setup/ticket-manager';
let latestBlock = 0;
export const blocks: BlockMap = {};
export const blocksByHash: {
    [hash: string]: number;
} = {};
export const readableBlocks: {
    [blockNumber: number | string]: ShardeumBlockOverride;
} = {};
...
const servicePointSpendHistory: {
    points: number;
    ts: number;
}[] = [];
...
function convertToReadableBlock(block: Block): ShardeumBlockOverride {
    ...
    if (previousBlock)
        defaultBlock.parentHash = previousBlock.hash;
...
export function getStakeTxBlobFromEVMTx(transaction: Transaction[TransactionType.Legacy] | Transaction[TransactionType.AccessListEIP2930]): unknown {
 ...
 *    ##     ##  ######   ######   #######   #######  ##    ##    ##       ##     ##  ######   ######  ########  ######
 */
async function createAccount(addressStr: string, stateManager: ShardeumState, balance: bigint = defaultBalance): Promise<WrappedEVMAccount> {
...
const configShardusEndpoints = (): void => {
    ...
    shardus.registerExternalGet('debug-points', debugMiddleware, async (req, res) => {
        try {
            ...
            res.json(`spent points: ${points} total:${debugLastTotalServicePoints}  ${Utils.safeStringify(servicePointSpendHistory)} `);
        }
        catch (error) {
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in debug-points endpoint:', error);
    ...
    shardus.registerExternalGet('debug-point-spenders', debugMiddleware, async (req, res) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in debug-point-spenders endpoint:', error);
            res.json({ error: error.message });
        }
    });
    shardus.registerExternalGet('debug-point-spenders-clear', debugMiddleware, async (req, res) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in debug-point-spenders-clear endpoint:', error);
            res.json({ error: error.message });
        }
    });
    ...
    shardus.registerExternalPost('inject', externalApiMiddleware, async (req, res) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in inject endpoint:', error);
    ...
    shardus.registerExternalGet('canUnstake/:nominee/:nominator', externalApiMiddleware, async (req, res) => {
        if (trySpendServicePoints(ShardeumFlags.ServicePoints['canUnstake/:nominee/:nominator'], req, 'canUnstake') === false) {
        ...
        try {
            ...
            const stakeUnlocked = isStakeUnlocked(nominator.account, nominatee.account, shardus, AccountsStorage.cachedNetworkAccount);
    ...
    shardus.registerExternalGet('eth_gasPrice', externalApiMiddleware, async (req, res) => {
        ...
        try {
            const result = calculateGasPrice(ShardeumFlags.baselineTxFee, ShardeumFlags.baselineTxGasUsage, await AccountsStorage.getCachedNetworkAccount());
    ...
    shardus.registerExternalPost('contract/call', externalApiMiddleware, async (req, res) => {
        ...
        if (trySpendServicePoints(ShardeumFlags.ServicePoints['contract/call'].endpoint, req, 'call-endpoint') ===
            false) {
        ...
        try {
            ...
            if (opt['to']) {
                ...
                if (accountIsRemote) {
                    ...
                    if (consensusNode != null) {
                        const postResp = await _internalHackPostWithResp(`${consensusNode.externalIp}:${consensusNode.externalPort}/contract/call`, callObj);
            ...
            if (trySpendServicePoints(ShardeumFlags.ServicePoints['contract/call'].direct, req, 'call-direct') ===
                false) {
                res.json({ result: null, error: 'node busy' });
                return;
            }
            ...
            if (callObj.block && callObj.block.number && callObj.block.timestamp) {
                ...
                if (callObj.block.useLatestState === false)
    ...
    shardus.registerExternalPost('contract/accesslist', externalApiMiddleware, async (req, res) => {
        ...
        if (trySpendServicePoints(ShardeumFlags.ServicePoints['contract/accesslist'].endpoint, req, 'accesslist') === false) {
            res.json({ result: null, error: 'node busy' });
            return;
        }
        try {
    ...
    shardus.registerExternalGet('debug-appdata/:hash', debugMiddleware, async (req, res) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in debug-appdata endpoint:', error);
    ...
    shardus.registerExternalGet('accounts', debugMiddlewareMedium, async (req, res) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in processing accounts request:', error);
            res.status(500).json({ error: error.message });
        }
    });
    shardus.registerExternalGet('genesis_accounts', externalApiMiddleware, async (req, res) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in processing genesis_accounts request:', error);
    ...
    shardus.registerExternalGet('secure_accounts', externalApiMiddleware, async (req, res) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in processing secure_accounts request:', error);
    ...
    shardus.registerExternalGet('system-info', debugMiddlewareLow, async (req, res) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in processing system-info request:', error);
            res.status(500).json({ error: error.message });
        }
    });
    shardus.registerExternalPut('query-certificate', externalApiMiddleware, async (req: Request, res: Response) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in processing query-certificate request:', error);
            res.status(500).json({ error: 'Internal Server Error' });
        }
    });
    ...
    shardus.registerExternalGet('debug-is-ready-to-join', async (req, res) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in processing debug-is-ready-to-join request:', error);
            res.status(500).json({ error: 'Internal Server Error' });
        }
    });
    ...
    shardus.registerExternalPut('admin-certificate', externalApiMiddleware, async (req, res) => {
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in processing admin-certificate request:', error);
            res.status(500).json({ error: 'Internal Server Error' });
        }
    });
...
const configShardusNetworkTransactions = (): void => {
    shardus.registerBeforeAddVerifier('nodeReward', async (txEntry: P2P.ServiceQueueTypes.AddNetworkTx<SignedNodeRewardTxData>) => {
    ...
    shardus.registerApplyVerifier('nodeReward', async (txEntry: P2P.ServiceQueueTypes.AddNetworkTx<SignedNodeRewardTxData>) => {
        const tx = txEntry.txData;
        const shardusAddress = tx.publicKey?.toLowerCase();
        const account = await shardus.getLocalOrRemoteAccount(shardusAddress);
        if (!account) {
    ...
    shardus.registerBeforeAddVerifier('nodeInitReward', async (txEntry: P2P.ServiceQueueTypes.AddNetworkTx<SignedNodeInitTxData>) => {
        const tx = txEntry.txData;
    ...
    shardus.registerApplyVerifier('nodeInitReward', async (txEntry: P2P.ServiceQueueTypes.AddNetworkTx<SignedNodeInitTxData>) => {
        const tx = txEntry.txData;
        const shardusAddress = tx.publicKey?.toLowerCase();
        const account = await shardus.getLocalOrRemoteAccount(shardusAddress);
        if (!account) {
    ...
    shardus.registerShutdownHandler('nodeInitReward', (node: P2P.NodeListTypes.Node, record: P2P.CycleCreatorTypes.CycleRecord) => {
    ...
    shardus.registerShutdownHandler('nodeReward', (node: P2P.NodeListTypes.Node, record: P2P.CycleCreatorTypes.CycleRecord) => {
 ...
 *    #### ##    ##    ##    ######## ##     ## ##    ## ##     ## ########       ##    ##     ##
 */
async function applyInternalTx(tx: InternalTx, wrappedStates: WrappedStates, txTimestamp: number): Promise<ShardusTypes.ApplyResponse> {
    ...
    if (internalTx.internalTXType === InternalTXType.ClaimReward) {
        ...
        try {
            await applyClaimRewardTx(shardus, claimRewardTx, wrappedStates, txId, txTimestamp, applyResponse, isAdminCertUnexpired);
        }
        catch (error) {
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in applyClaimRewardTX', error);
            shardus.applyResponseSetFailed(applyResponse, `applyClaimRewardTX failed for nominee: ${claimRewardTx.nominee}, reason: ${error?.message ?? error}`);
        }
    }
    if (internalTx.internalTXType === InternalTXType.Penalty) {
        ...
        try {
            ...
            /* prettier-ignore */ if (logFlags.error)
                console.error('Error in applyPenaltyTX', error);
...
function setGlobalCodeByteUpdate(txTimestamp: number, wrappedEVMAccount: WrappedEVMAccount, applyResponse: ShardusTypes.ApplyResponse): void {
...
const getOrCreateBlockFromTimestamp = (timestamp: number, scheduleNextBlock = false): Block => {
    ...
    const newBlockTimestampInSecond = cycle.start +
        cycle.duration +
...
async function estimateGas(injectedTx: {
    from: string;
    maxFeePerGas: string;
    gas: number;
} & LegacyTxData): Promise<{
    estimateGas: string;
}> {
    ...
    const transaction: LegacyTransaction | AccessListEIP2930Transaction = TransactionFactory.fromTxData<TransactionType.Legacy>(txData);
    ...
    preRunTxState._transactionState.insertFirstAccountReads(callerEVMAddress, callerAccount ? callerAccount.account : fakeAccount);
    const customEVM = new EthereumVirtualMachine({
        common: evmCommon,
...
async function generateAccessList(injectedTx: ShardusTypes.OpaqueTransaction, warmupList: {
    accessList: any[];
    codeHashes: CodeHashObj[];
}, caller: string): Promise<{
    ...
    try {
        ...
        for (const [codeHash, contractByteWrite] of readAccounts.contractBytes) {
            ...
            if (!allInvolvedContracts.includes(contractAddress))
                allInvolvedContracts.push(contractAddress);
        ...
        for (const address of allInvolvedContracts) {
            ...
            for (const [codeHash, byteReads] of readAccounts.contractBytes) {
                ...
                if (!allCodeHash.has(contractAddress))
                    allCodeHash.set(contractAddress, { codeHash, contractAddress });
            }
...
async function fetchAndCacheAccountData(shardusAddress: string, warmupCache: Map<string, WrappedEVMAccount>, warmupStats: WarmupStats, useRICache: boolean, txid: string, type: AccountType): Promise<void> {
    warmupStats.accReq++;
    const startTime = Date.now();
...
function getNodeCountForCertSignatures(): number {
    ...
    if (latestCycles && latestCycles.length > 0)
        [latestCycle] = latestCycles;
...
//         /* prettier-ignore */ console.log(`[event_loop:blocked] request id: ${uuid} Complete Stack: ${Utils.safeStringify(stack)}`);
//       }
//     },
...
const shardusSetup = (): void => {
    ...
    shardus.setup({
        ...
        async apply(timestampedTx: ShardusTypes.OpaqueTransaction, wrappedStates, originalAppData) {
            ...
            if (shardeumState.usedByApply === true) {
                if (ShardeumFlags.cleanStaleShardeumStateMap) {
                    ...
                    /* prettier-ignore */ if (logFlags.error)
                        console.error(`shardeumState.usedByApply === true clearing shardeumState! ${txId}`);
            ...
            if (appData.internalTx && appData.internalTXType === InternalTXType.Stake) {
                ...
                if (ShardeumFlags.useAccountWrites) {
                    ...
                    const wrappedChangedNodeAccount = WrappedEVMAccountFunctions._shardusWrappedAccount(wrappedStates[nomineeNodeAccount2Address].data as NodeAccount2);
                    // for nominee node account
                    shardus.applyResponseAddChangedAccount(applyResponse, nomineeNodeAccount2Address, wrappedChangedNodeAccount as ShardusTypes.WrappedResponse, txId, txTimestamp);
                }
                const blockForReceipt = getOrCreateBlockFromTimestamp(txTimestamp);
                ...
                const readableReceipt: ReadableReceipt = {
                    ...
                    nonce: bigIntToHex(transaction.nonce),
                    blockHash: readableBlocks[blockNumberForTx].hash, // eslint-disable-line security/detect-object-injection
                    cumulativeGasUsed: bigIntToHex(scaleByStabilityFactor(BigInt(ShardeumFlags.constantTxFeeUsd), AccountsStorage.cachedNetworkAccount)),
                    gasUsed: bigIntToHex(scaleByStabilityFactor(BigInt(ShardeumFlags.constantTxFeeUsd), AccountsStorage.cachedNetworkAccount)),
                    gasRefund: '0x0',
                    gasPrice: bigIntToHex(transaction.gasPrice),
                    gasLimit: bigIntToHex(transaction.gasLimit),
                    maxFeePerGas: undefined,
            ...
            if (appData.internalTx && appData.internalTXType === InternalTXType.Unstake) {
                ...
                if (ShardeumFlags.useAccountWrites) {
                    ...
                    const wrappedChangedNodeAccount = WrappedEVMAccountFunctions._shardusWrappedAccount(wrappedStates[nomineeNodeAccount2Address].data as NodeAccount2);
                    // for nominee node account
                    shardus.applyResponseAddChangedAccount(applyResponse, nomineeNodeAccount2Address, 
                    // eslint-disable-next-line security/detect-object-injection
                    wrappedChangedNodeAccount as ShardusTypes.WrappedResponse, txId, txTimestamp);
                }
                const blockForReceipt = getOrCreateBlockFromTimestamp(txTimestamp);
                let blockNumberForTx = blockForReceipt.header.number.toString();
                if (ShardeumFlags.supportInternalTxReceipt === false) {
            ...
            for (const accountId in wrappedStates) {
                ...
                if (ShardeumFlags.VerboseLogs) {
                    ...
                    const { homePartition } = __ShardFunctions.addressToPartition(shardus.stateManager.currentCycleShardData.shardGlobals, shardusAddress);
            ...
            const { accounts: accountWrites, contractStorages: contractStorageWrites, contractBytes: contractBytesWrites, } = shardeumState._transactionState.getWrittenAccounts();
            ...
            if (accountWrites.size === 0) {
                ...
                for (const accountId in wrappedStates) {
                    if (wrappedStates[accountId].timestamp === 0)
                        continue;
        ...
        async txPreCrackData(tx, appData): Promise<{
            status: boolean;
            reason: string;
        }> {
            ...
            if (isInternalTx(tx) === false && isDebugTx(tx) === false) {
                ...
                if (ShardeumFlags.autoGenerateAccessList === false)
                    shouldGenerateAccesslist = false;
                else if (isStakeRelatedTx)
                    shouldGenerateAccesslist = false;
                ...
                else if (remoteTargetAccount == null && appData.newCAAddr == null)
                    shouldGenerateAccesslist = false;
                ...
                if (shouldGenerateAccesslist) {
                    ...
                    if (success === true) {
                        ...
                        const { shardusMemoryPatterns, failedAccessList, accessList: generatedAccessList, codeHashes, } = await generateAccessList(tx, appData?.warmupList, 'txPrecrackData');
        ...
        crack(timestampedTx, appData) {
            ...
            try {
                ...
                result.allKeys = result.allKeys.concat(result.sourceKeys, result.targetKeys, otherAccountKeys, result.storageKeys, additionalAccounts, result.codeHashKeys);
            }
            catch (e) {
            }
            return {
        ...
        async setAccountData(accountRecords) {
            ...
            for (const account of accountRecords) {
                ...
                const shardusAddress = getAccountShardusAddress(wrappedEVMAccount);
                if (wrappedEVMAccount.accountType !== AccountType.NetworkAccount &&
        ...
        async getRelevantData(accountId, timestampedTx, appData) {
            ...
            if (!Object.prototype.hasOwnProperty.call(tx, 'raw'))
                throw new Error('getRelevantData: No raw tx');
                ...
                if (appData.internalTXType === InternalTXType.Stake) {
                    if (!wrappedEVMAccount) {
                        ...
                        if (accountId === stakeTxBlob.nominee) {
                            ...
                            accountCreated = true;
                            return shardus.createWrappedResponse(accountId, accountCreated, nodeAccount.hash, nodeAccount.timestamp, nodeAccount);
                        }
        ...
        transactionReceiptPass(timestampedTx: ShardusTypes.OpaqueTransaction, wrappedStates: {
            [id: string]: WrappedAccount;
        }, applyResponse: ShardusTypes.ApplyResponse, isExecutionGroup: boolean) {
            //@ts-ignore
            const { tx } = timestampedTx;
        ...
        transactionReceiptFail(timestampedTx: ShardusTypes.OpaqueTransaction, wrappedStates: {
            ...
            if (ShardeumFlags.expiredTransactionStateFix === false)
                return;
            //@ts-ignore
            const { tx } = timestampedTx;
            const txId: string = generateTxId(tx);
        ...
        async isReadyToJoin(latestCycle: ShardusTypes.Cycle, publicKey: string, activeNodes: P2P.P2PTypes.Node[], mode: P2P.ModesTypes.Record['mode']): Promise<boolean> {
            ...
            if (initialNetworkParamters && networkAccount) {
                if (!isValidVersion(networkAccount.data.current.minVersion, networkAccount.data.current.latestVersion, version)) {
                ...
                if (networkAccount.data.mode === ServerMode.Release &&
        ...
        async eventNotify(data: ShardusTypes.ShardusEvent) {
            ...
            else if (eventType === 'node-left-early' &&
            ...
            else if (eventType === 'node-sync-timeout' &&
                AccountsStorage.cachedNetworkAccount.current.enableNodeSlashing === true &&
        ...
        await (async (): Promise<void> => {
            ...
            async function networkMaintenance(): Promise<NodeJS.Timeout> {
                ...
                if (logFlags.dapp_verbose)
                    shardus.log('New maintenance cycle has started');
