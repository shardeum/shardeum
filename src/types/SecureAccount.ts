...
export function deserializeSecureAccount(stream: VectorBufferStream): SecureAccount {
    ...
    const minimumBytesNeeded = 8 + // id (String)
        8 + // hash (String)
    ...
    try {
        ...
        if (typeof nextTransferAmount !== 'bigint')
            throw new Error('nextTransferAmount must be bigint');
        const nextTransferTime = Number(stream.readBigUInt64());
        if (isNaN(nextTransferTime))
            throw new Error('nextTransferTime must be number');
