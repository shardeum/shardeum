...
export function deserializeWrappedEVMAccount(stream: VectorBufferStream): WrappedEVMAccount {
    ...
    const readableReceipt = stream.readUInt8() === 1 ? (Utils.safeJsonParse(stream.readString()) as ReadableReceipt) : undefined;
    const operatorAccountInfo = stream.readUInt8() === 1 ? (Utils.safeJsonParse(stream.readString()) as OperatorAccountInfo) : undefined;
