...
- It's okay to commit and push frequently to your branch as long as the branch is rebase and commit are squash before PR.
...
- Branches must be tested at least with smoke test manually.
- Must include test coverage.
- Must pass CI/CD pipelines.
- Reqularly pull updates from development branch `dev` by `--rebase`.
